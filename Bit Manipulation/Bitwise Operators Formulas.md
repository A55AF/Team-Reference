...
- $A + B = (A \oplus B) + 2 * (A \  \& \ B)$
- $A + B = (A\  | \ B) + (A \& B)$
- $A + B = (A \oplus B) + 2 * (A\  \& \ \sim{B})$  \[When carries are shifted\]
...
- XOR:  $A \oplus B = (A + B) - 2 * (A\  \& \ B)$
- AND:  $A\  \& \ B = ((A + B) - (A \oplus B)) / 2$
- OR:   $A\  | \ B = ((A + B) + (A \oplus B)) / 2$
...
- Flip all bits:               x ^ ALL_ONES  (or ~x)
- Flip k-th bit (0-indexed):   x ^ (1 << k)

### Rightmost Bit Manipulations
- Toggle rightmost set bit:    n & (n - 1)
- Isolate rightmost set bit:   n & (-n)       or  n & ~(n - 1)
- Isolate rightmost zero bit:  ~n & (n + 1)
...
### Range XOR Formula
XOR_Sum(L to R) = PrefixXOR(R) ^ PrefixXOR(L - 1)
...
# Bitwise XOR ($\oplus$) Master Cheat Sheet
